...
export const useThemeToggle = ({
  ...
  // Sync isDark state after hydration
  useEffect(() => {
  ...
  const toggleTheme = useCallback(() => {
    ...
    const switchTheme = () => {
      const newTheme = nextThemeIsDark ? "dark" : "light";
      document.documentElement.setAttribute("data-theme", newTheme);
      localStorage.setItem("theme", newTheme);
      
      // Dispatch custom event if other Astro components listen to it
...
export const ThemeToggleButton = ({
  ...
  return (
    <button
      type="button"
      className={cn(
        "size-[34px] cursor-pointer rounded-full p-0 transition-all duration-300 active:scale-95 flex items-center justify-center border-none",
        "bg-transparent text-[var(--color-muted)] hover:text-[var(--color-ink)] hover:bg-[rgba(255,255,255,0.08)]",
